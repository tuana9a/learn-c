...
void readMapInFileNice(FILE* inputFile, int  originMap[][width]) {
	...
	for (int i = 0;i < height;++i) {
		for (int j = 0;j < width;++j) {
			originMap[i][j] = 0;//0 la bi chan -1 la di duoc
		}
	}
	tempCol++;
...
void printMapNice(int mapToPrint[][width]) {
	for (int i = 0;i < height;++i) {
		for (int j = 0;j < width;++j) {
			int mark = mapToPrint[i][j];
			switch (mark) {
			case 0: printf("\u2588\u2588\u2588");break;// 0 la bi chan
			case -1:printf("   ");break;// -1 la di duoc
			default:printf("%3d", mark);break;// buoc di				
			}
		}
		printf("\n");
	}
...
void tryAll(int currentMap[height][width], int currentX, int currentY, int currentStep) { //X,Y bat dau tu 0, X:ngang   Y:doc
	...
	tryAll(tempMap[0], currentX + 1, currentY, currentStep + 1);
	tryAll(tempMap[1], currentX - 1, currentY, currentStep + 1);
	tryAll(tempMap[2], currentX, currentY + 1, currentStep + 1);
	tryAll(tempMap[3], currentX, currentY - 1, currentStep + 1);
}


int main(int argc, char const* argv[]) {
	FILE* inputFile = fopen("maze/examples/map.txt", "r");
